...
// constant defination
using USB_BOS_DESCRIPTOR = struct _USB_BOS_DESCRIPTOR
{
...
using USB_BOS_USB20_DEVICE_EXTENSION = struct _USB_BOS_USB20_DEVICE_EXTENSION
{
   UCHAR bLength;            /* Descriptor length*/
   UCHAR bDescriptorType;    /* Descriptor Type */
...
using USB_BOS_SS_DEVICE_CAPABILITY = struct _USB_BOS_SS_DEVICE_CAPABILITY
{
   UCHAR  bLength;            /* Descriptor length*/
   UCHAR  bDescriptorType;    /* Descriptor Type */
   UCHAR  bDevCapabilityType; /* Device capability type*/
   UCHAR  bmAttribute;        // Bitmap encoding for supprted feature and  Link power managment supprted if set
   USHORT wSpeedsSuported; // low speed supported if set,full speed supported if set,high speed supported if set,super
                           // speed supported if set,15:4 nt used
...
using USB_BOS_CONTAINER_ID = struct _USB_BOS_CONTAINER_ID
{
   UCHAR bLength;                                                /* Descriptor length*/
   UCHAR bDescriptorType;                                        /* Descriptor Type */
   UCHAR bDevCapabilityType;                                     /* Device capability type*/
...
}, *PUSB_BOS_CONTAINER_ID;

using USB_SUPERSPEED_ENDPOINT_COMPANION_DESCRIPTOR = struct _USB_SUPERSPEED_ENDPOINT_COMPANION_DESCRIPTOR
{
   UCHAR  bLength;
