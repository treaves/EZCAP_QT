 ...
 * usb vendor request command
 */
constexpr int QHYCCD_REQUEST_WRITE(0x40);

constexpr int MACHANICALSHUTTER_OPEN(0);
constexpr int MACHANICALSHUTTER_CLOSE(1);
constexpr int MACHANICALSHUTTER_FREE(2);

constexpr int MAX_READMODE_NUMBER(8);
constexpr int MAX_READMODE_CAMARA_NUMBER(8);
...
using QHYCamMinMaxStepValue = struct _QHYCamMinMaxStepValue
{
   const char * name;
   double       min;
   double       max;
...
using QHYGetImageParam = struct _QHYGetImageParam
{
   void *    handle;
   uint8_t * imgdata;
   uint32_t  w;
...
using QHYCCDProcCallBack = QHYDWORD (*)(void *, QHYDWORD, QHYDWORD, QHYDWORD);
#endif
