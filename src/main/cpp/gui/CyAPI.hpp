...
using vendorCmdData = struct vendorCmdData
{
...
#define __USB200_H__
#pragma pack(push, 1)
using USB_DEVICE_DESCRIPTOR = struct _USB_DEVICE_DESCRIPTOR
{
   UCHAR  bLength;
...
using USB_ENDPOINT_DESCRIPTOR = struct _USB_ENDPOINT_DESCRIPTOR
{
   UCHAR  bLength;
   UCHAR  bDescriptorType;
...
using USB_CONFIGURATION_DESCRIPTOR = struct _USB_CONFIGURATION_DESCRIPTOR
{
   UCHAR  bLength;
   UCHAR  bDescriptorType;
   USHORT wTotalLength;
...
using USB_INTERFACE_DESCRIPTOR = struct _USB_INTERFACE_DESCRIPTOR
{
   UCHAR bLength;
   UCHAR bDescriptorType;
...
using USB_STRING_DESCRIPTOR = struct _USB_STRING_DESCRIPTOR
{
   UCHAR bLength;
   UCHAR bDescriptorType;
   WCHAR bString[1];
...
using USB_COMMON_DESCRIPTOR = struct _USB_COMMON_DESCRIPTOR
{
   UCHAR bLength;
   UCHAR bDescriptorType;
...
using CTL_XFER_TGT_TYPE        = enum { TGT_DEVICE, TGT_INTFC, TGT_ENDPT, TGT_OTHER };
using CTL_XFER_REQ_TYPE        = enum { REQ_STD, REQ_CLASS, REQ_VENDOR };
using CTL_XFER_DIR_TYPE        = enum { DIR_TO_DEVICE, DIR_FROM_DEVICE };
using XFER_MODE_TYPE           = enum { XMODE_BUFFERED, XMODE_DIRECT };

const int MAX_ENDPTS           = 32;
const int MAX_INTERFACES       = 255;
const int USB_STRING_MAXLEN    = 256;

using FX3_FWDWNLOAD_MEDIA_TYPE = enum { RAM = 1, I2CE2PROM, SPIFLASH };
using FX3_FWDWNLOAD_ERROR_CODE = enum {
   SUCCESS = 0,
   FAILED,
   INVALID_MEDIA_TYPE,
   INVALID_FWSIGNATURE,
...
public:
   ...
   CCyUSBEndPoint(HANDLE                                         hDev,
                  PUSB_ENDPOINT_DESCRIPTOR                       pEndPtDescriptor,
   ...
   virtual bool   FinishDataXfer(PUCHAR          buf,
                                 LONG &          len,
                                 OVERLAPPED *    ov,
                                 PUCHAR          pXmitBuf,
                                 CCyIsoPktInfo * pktInfos = NULL);
...
public:
   CCyUSBEndPoint * EndPoints
     [MAX_ENDPTS]; /* Holds pointers to all the interface's endpoints,
